        ...
        if let Some(version) = versions.get(pos) {
            ...
            if let Some(bin) = msg {
                ...
                if config.heroes.len() == 3 {
                    // TODO: modify the copy/paste job get_rand_slice and create a stateful Prng to repeatedly
                    // call next_u32 as needed, instead of re-creating the Prng like is being done now
                    let mut rand_slice =
